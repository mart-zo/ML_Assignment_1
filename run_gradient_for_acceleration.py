...
def gradient_descent(start, function, gradient, learn_rate, max_iter, tol=0.0000001):
    """
...
def grad_x_a(alpha):
    ...
    da1 = -2 * np.sum(T * (y_x - predicted))
    da2 = -2 * np.sum(T**2 * (y_x - predicted))
...
def grad_y_a(alpha):
    ...
    da2 = -2 * np.sum(T**2 * (y_y - predicted))
    return np.array([da0, da1, da2])
...
def grad_z_a(alpha):
    ...
    da2 = -2 * np.sum(T**2 * (y_z - predicted))
    return np.array([da0, da1, da2])


# Run the gradient descent algorithm starting from x = 0 - for acceleration
print("X dimension")
alpha_x_a = gradient_descent(np.zeros(3), sse_x_a, grad_x_a, learn_rate=0.000001, max_iter=10000000)

print("\nY dimension")
alpha_y_a = gradient_descent(np.zeros(3), sse_y_a, grad_y_a, learn_rate=0.000001, max_iter=10000000)

print("\nZ dimension")
alpha_z_a = gradient_descent(np.zeros(3), sse_z_a, grad_z_a, learn_rate=0.000001, max_iter=10000000)
